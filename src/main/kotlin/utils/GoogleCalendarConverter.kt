...
import utils.GoogleCalendarConverter.toBookingDTO
...
object GoogleCalendarConverter {
    fun Event.toBookingDTO(): BookingDTO {
        ...
        return BookingDTO(owner = getUser(email),
    ...
    private fun getWorkspace(email: String): WorkspaceDTO {
        TODO("тут достаём воркспейс по гугловкому id")
    }
    ...
    fun BookingDTO.toGoogleEvent(): Event {
        ...
        return Event().apply {
            ...
            description = "${dto.owner.integrations?.first { it.id == "email" }} - почта организатора"
    ...
    private fun UserDTO.toAttendee(): EventAttendee {
        return EventAttendee().also {
            it.email = this.integrations?.first { it.name == "email" }?.value //TODO надо допелить получение почты
        }
    }
    ...
    private fun UserDTO.toGoogleOrganizer(): Event.Organizer? {
        return Organizer().also {
            it.email =
                this.integrations?.first { integ -> integ.name == "email" }?.value //TODO надо допелить получение почты
        }
    }

    private fun WorkspaceDTO.toAttendee(): EventAttendee {
        return EventAttendee().also {
            it.email = getCalendarIdByName(name) //TODO надо допелить получение комнаты
            it.resource = true
        }
    }

    private fun getCalendarIdByName(name: String) =
        when (name) {
